...
def create_network(opt):
	...
	#generator
	with tf.variable_scope("generatorA"):	
	...
	with tf.variable_scope("discriminator_loss"):
		d_1_loss = tf.reduce_mean(-(tf.log(d_1_real + EPS) + tf.log(1 - d_1_fake + EPS)))
		d_2_loss = tf.reduce_mean(-(tf.log(d_2_real + EPS) + tf.log(1 - d_2_fake + EPS)))

	with tf.variable_scope("generator_loss"):
		g_1_loss_GAN = tf.reduce_mean(-tf.log(d_1_fake + EPS))
		g_1_loss_L1 = tf.reduce_mean(tf.abs(blur_B - fake_blur_B))
		g_1_loss = g_1_loss_GAN  + g_1_loss_L1 * lambda_A

		g_2_loss_GAN = tf.reduce_mean(-tf.log(d_2_fake + EPS))
		g_2_loss_L1 = tf.reduce_mean(tf.abs(input_B - fake_B))
		g_2_loss = g_2_loss_GAN  + g_2_loss_L1 * lambda_A

	#tensorboard summary
	g_1_loss_L1_summary = tf.summary.scalar("g_1_loss_L1", g_1_loss_L1)
	g_1_loss_GAN_summary = tf.summary.scalar("g_1_loss_GAN", g_1_loss_GAN)
	d_1_loss_sum = tf.summary.scalar("d_1_loss", d_1_loss)

	g_2_loss_L1_summary = tf.summary.scalar("g_2_loss_L1", g_2_loss_L1)
	g_2_loss_GAN_summary = tf.summary.scalar("g_2_loss_GAN", g_2_loss_GAN)
	d_2_loss_sum = tf.summary.scalar("d_2_loss", d_2_loss)
	...
	d_1_solver = tf.train.AdamOptimizer(starter_learning_rate, 0.5).minimize(d_1_loss, var_list=tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope='discriminatorA'))
	g_1_solver = tf.train.AdamOptimizer(starter_learning_rate, 0.5).minimize(g_1_loss, var_list=tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope='generatorA'))
	
	d_2_solver = tf.train.AdamOptimizer(starter_learning_rate, 0.5).minimize(d_2_loss, var_list=tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope='discriminatorB'))
	g_2_solver = tf.train.AdamOptimizer(starter_learning_rate, 0.5).minimize(g_2_loss, var_list=tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope='generatorB'))
